...
int main(int argc, char** argv) {
 ...
 // Declare the node parameters
  ...
  std::unordered_map<std::string, std::string> ompl_map = {
    {"ompl", "rm_75_arm[RRTConnectkConfigDefault]"}
  };
  ...
  robot->getJointModelGroup("rm_75_arm")->getVariableDefaultPositions("home_pose", target_state);
  target_state["joint1"] = +TAU / 8;
  RCLCPP_INFO(logger, "Target state set for 'rm_75_arm' group");
  ...
  initial_scene->getCurrentStateNonConst().setToDefaultValues(robot->getJointModelGroup("rm_75_arm"), "home_pose");
  ...
  // First initial state option: 90 degree offset from target goal
  {
    ...
    scene->getCurrentStateNonConst().setVariablePositions({ { "joint1", -TAU / 8 } });
