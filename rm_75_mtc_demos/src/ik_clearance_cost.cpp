 ...
 *
 * This program sets up a motion planning task for a rm_75 robot rm_75_arm using MoveIt Task Constructor.
...
int main(int argc, char** argv) {
  ...
  // Load the robot model (rm_75)
  t.loadRobotModel(node);
  assert(t.getRobotModel()->getName() == "rm_75");
  ...
  // Set the rm_75_arm to its "ready" position
  [[maybe_unused]] bool found =
      robot_state.setToDefaultValues(robot_state.getJointModelGroup("rm_75_arm"), "ready");
  assert(found);
  RCLCPP_INFO(logger, "rm_75_arm set to 'ready' position");
