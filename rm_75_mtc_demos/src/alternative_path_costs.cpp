...
int main(int argc, char** argv) {
    ...
    assert(t.getRobotModel()->getName() == "rm_75_arm");
    ...
    robot_state.setToDefaultValues(robot_state.getJointModelGroup("rm_75_arm"), "home_pose");
    ...
    // Strategy 1: Minimize path length
    {
      auto connect{ std::make_unique<stages::Connect>(
           "path length", stages::Connect::GroupPlannerVector{ { "rm_75_arm", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "trajectory duration", stages::Connect::GroupPlannerVector{ { "rm_75_arm", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "eef motion", stages::Connect::GroupPlannerVector{ { "rm_75_arm", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("Link7")); // Distance traveled by the link in meters
      alternatives->add(std::move(connect));
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "elbow motion", stages::Connect::GroupPlannerVector{ { "rm_75_arm", pipeline } }) };
    ...
    // In our goal setup, we want the robot's rm_75_arm and gripper to be in the 'ready' position."
    auto goal_scene{ scene->diff() };
    goal_scene->getCurrentStateNonConst().setToDefaultValues(robot_state.getJointModelGroup("rm_75_arm"), "pose1");
    std::cout << "Created goal scene with 'pose1' position" << std::endl;
